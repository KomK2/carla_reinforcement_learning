...
class CarEnv(gym.Env):
	...
	def __init__(self):
		...
		self.cnn_model = load_model('/home/kiran/projects/test/test2/Carla/working_rl/model_saved_from_CNN.h5',compile=False)
		self.cnn_model.compile()

		self.collision_hist = []
		self.lane_invade_hist = []
		self.reward_log = []
		self.distance_travelled_log = []
		self.steering_log = []
		self.speed_log = []
	...
	def step(self, action):
		...
		if self.SHOW_CAM:
            ...
            carla.Transform(
                trans.location + carla.Location(x=0.0, z=3.0, y= -8.0),  # Place behind the car at some height
                carla.Rotation(pitch=0.1, yaw=trans.rotation.yaw )  # Align with the car's yaw
            )
        )
		...
		self.distance_travelled_log.append(distance_travelled)

		# Log steering action
		steer = action[0]
		self.steering_log.append(steer	)	

		# Log speed
		self.speed_log.append(kmh)

		return self.image_for_CNN, reward, done, {}

	def reset(self):
		self.reward_log.clear()
		self.distance_travelled_log.clear()
		self.steering_log.clear()
